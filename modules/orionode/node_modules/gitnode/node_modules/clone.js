...
{
    this.objects = this.objects.concat(lastSD.objs);
    if(lastSD.capabilites)
    {
...
var parseRefDiscHelper = function(data, lastServerDiscovery)
{
    ...
    lastServerDiscovery.merge(newServerDiscovery.objects);
...
var checkForStop = function(data)
{
...
var gitCloneConversation = function(gitProtoLink)
{
    var ans = [];
    var sDiscovery = new serverDiscovery();
    ...
    client.on('data', function(data){
        //console.log(data.toString());
        ans.push(data);
        if(checkForStop(data))
        {
            //
            //BELOW DOESEN'T WORK, WHY????
//            ans.forEach(function(element){
//                sDiscovery = parseRefDiscHelper(element, sDiscovery);
//                console.log(sDiscovery.rest);
//            });
            //I THINK THIS WORKS:
            sDiscovery = parseReferenceDiscovery(Buffer.concat(ans));
            client.end();
        }
    });
    client.on('end', function(){
       //sDiscovery = parseRefDiscHelper(data, sDiscovery);
