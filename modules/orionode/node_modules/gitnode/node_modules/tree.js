...
Tree.prototype.toFile = function () {
    ...
    for (var filename in this.entries) {
        var sha1 = this.entries[filename]['sha1'];
        var sha1Buff = utils.convertHexToDigest(sha1);
        var entryStr = this.entries[filename]['mode'] + ' ' + filename;
        var entryBuff;
        entryStr = entryStr + '\0';
 ...
 * @param callback function(err, this.entries)
 * this.entries is dictionary of the following format
 * {filename : {'sha1' : sha1, 'mode' : mode, 'type' : type}};
 */
...
Tree.prototype.walk = function (repodir, headSha1, done) { 
    var error = false; //if there's an error we want to callback only once..
    var graph = {};
    graph[headSha1] = this.entries;
    var doJob = function (tree, repodir, recursiveDir, callback) {
        var filenames = {};
        ...
        keys.forEach(function(file) {
            ...
            if (treeCurrent.entries[file]['type'] === 'tree') {
                readFileToTree(treeCurrent.entries[file]['sha1'], repodir, function(err, tree) {
                    if(error) {
                        return;
                    }
                    if(err) {
                        ...
                        graph[treeCurrent.entries[file]['sha1']] = tree.entries;
                        doJob(tree, repodir, recursiveDir + file + '/', function(err, res) {
                            if(err) ;
                            else {
                                for (var attrname in res) { filenames[attrname] = res[attrname]; }
                                if (!--pending) callback(null, filenames, graph, headSha1);
                            }
                        });
                    }
                });
    ...
    doJob(this, repodir, '', done);
};
/*
function saveGraphTree(repopath, graph, graphStart, done) {
    graph[graphStart]
    for(var entry in graph[graphStart]) {
