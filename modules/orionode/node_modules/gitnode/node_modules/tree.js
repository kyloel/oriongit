...
/* 
 * repodir is used to read next tree from the repo, when we have its sha1
 * For example: repodir -> '/home/marcinek/gitrepotest/'
 * 
    ...
    var doJob = function (tree, repodir, recursiveDir, callback) {
        ...
        keys.forEach(function(file) {
            ...
            if (treeCurrent.entries[file]['type'] === 'tree') {
                readFileToTree(treeCurrent.entries[file]['sha1'], repodir + '.git/', function(err, tree) {
                    if(error) {
                        return;
                    }
                    if(err) {
